...
std::string getUserWrittenType(const clang::DeclStmt *DeclStmt,
  ...
  std::string FVStr =
      Lexer::getSourceText(CharSourceRange::getTokenRange(FVLoc), SM, LangOpts);
...
void OneNamePerDeclarationCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (const auto *DeclStmt =
    ...
    if (DeclStmt->isSingleDecl() == false) {
      ...
      std::string UserWrittenType = getUserWrittenType(DeclStmt, SM, LangOpts);
        ...
        if (const clang::DeclaratorDecl *DecDecl =
          ...
          if (const clang::VarDecl *VarDecl =
            ...
            if (VarDecl->getType().getCanonicalType()->isScalarType() &&
              ...
              auto PP = tidy::utils::lexer::findTokenAfterLocation(
                  VariableLocation.getEnd(), *Result.Context, true,
                  {tok::r_paren});
              SingleDeclaration += Lexer::getSourceText(
          ...
          VariableLocation.setBegin(
              tidy::utils::lexer::findTokenAfterLocation(
                  VariableLocation.getEnd(), *Result.Context, true, tokens)
                  .getLocWithOffset(1));
        }
      }
